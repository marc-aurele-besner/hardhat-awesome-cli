...
async function main() {
    ...
    if (hre.network.name !== 'hardhat' && hre.network.name !== 'local') {
        ...
        if (!logicContract)
            logicContract = await hre.addressBook.retrieveContract('MockERC721Upgradeable', hre.network.name);
        if (!logicContract)
            logicContract = await hre.addressBook.retrieveContract('MockERC1155Upgradeable', hre.network.name);
    }
    if (!logicContract) {
